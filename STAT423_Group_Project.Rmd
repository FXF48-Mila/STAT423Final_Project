...
## Initial Thought: Full Regression Model, Data Processing -- Log Transformation
...
mtext("Figure 1: Log Transformation on Health Insurance Charges", outer = TRUE, side = 1, line = 2, cex = 0.8, font = 3)
...
### Fit Full Models and Assumption Checks
...
# Set up the layout for the base R plots
par(mfrow = c(2, 2))

# Create the base R plots and save them in a list
plot_list <- list()
...
mtext("Figure 2: Assumption Check For Full Model", outer = TRUE, side = 1, line = 2, cex = 0.8, font = 3)
...
We can clearly see that there exist two regression lines in Figure 3 for `age` and `charges`.
...
## Alternative Thought: Split The Dataset Based on Smoke Status
...
mtext("Figure 5", outer = TRUE, side = 1, line = 2, cex = 0.8, font = 3)
...
caption <- textGrob(label = "Figure 8", x = 0.5, y = 0.99, just = "center", gp = gpar(fontsize = 9, fontface = 'italic'))

grid.arrange(plot, caption)
```
