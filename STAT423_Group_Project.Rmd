...
Therefore, we would consider that we have successfully deleted the outliers that are irrelevant to our model. The linear regression model would be:
...
$$
such that for each $\hat{\epsilon}_i^*$,
$$
...
via
$$
X_i^* = X_i, \quad Y_i^* = \hat{\beta}_0 + \hat{\beta}_1X_i + \hat{\epsilon}_i^*.
$$
...
p + labs(caption = "Figure 11: Confidence Intervals for Model Coefficients") +
  theme(plot.caption = element_text(face = "italic"))
```
...
For predictions, we would consider in the test set that we still use the 80% quantile of the empirical residuals from the initial regression model in the training set as the threshold to judge if the given data is outlier or not. That is to say,
$$\hat Y=\{\hat Y_i:\hat\epsilon_i<Q_{0.85\hat\epsilon_{train}}\}.$$
...
plot + labs(caption = "Figure 12: Comparison of model predictions with actual values") +
  theme(plot.caption = element_text(face = "italic"))
```
...
From the above analysis, for the smokers, the result showed that bmi is a strong predictor of smokers' insurance charges. The prediction model shows a linear relationship between the actual and predicted charges. For nonsmokers, our improved method, which included removing outliers and residual scaling, found that age, children, regionsoutheast and regionsouthwest are statistically significant predictors, while bmi and regionnorthwest are not statistically significant predictors. Using the 80% quantile of the empirical residuals in the model, it appears to predict the charges well. These results show that health insurance costs are complicated and that it's important for prediction models to take into account personal traits.
Based on what we learned from this research, we should use more advanced predictive modelling methods and keep improving the models to better reflect how healthcare use and prices change over time. The clear effect that smoking has on insurance rates shows how important it is to have custom models that take living factors into account. As health care needs and insurance policies change, it will be important to use machine learning algorithms and keep models up to date with new data to make predictions more accurate and make sure they stay relevant to current trends.
