...
We realize that splitting the dataset based on smoking status may alter the distribution of the response variable: charges. Therefore, we transformed the dataset separately for each smoke status group to ensure our modeling is appropriate for these differences and then produce a reliable analysis. After the box-cox analysis, we chose to do a square root transformation to the charges for smoker dataset and a log transformation to the charges for non-smoker dataset (See the plot in Appendix).
...
Since the non-smoker uses the bootstrap method which would be introduced below, it is robotic and is resistant to the imbalanced data distribution. Therefore we still consider the children number to be numerical.
...
Then, we fit a full model as `log_charges ~ age + sex + bmi + children + smoker + region` to see how personal information is related to the charges. We need to check the assumption for our initial model first.
```{r}
fit.full <- lm(log_charges ~ age + sex + bmi + children + smoker + region, data = train_set1)
#summary(fit.full)
```

```{r fig.height=2.9, fig.width=10}
par(mfrow=c(1, 4), oma = c(3, 0, 0, 0), mar = c(0, 4, 3, 0.5), cex = 1)
...
par(mfrow = c(1, 1), oma = c(0, 0, 0, 0))
```
In the residual vs. fitted plot, the points are not randomly around the horizontal line at zero. We can clearly see some trend in the TA plot, suggesting that the linearity assumption is violated. Since we transformed the response variable, we use the standard residuals vs. fitted plot to check the homoscedasticity. In the Scale-Location plot, the spread of points is not constant across all values of the fitted values, which indicates that the assumption of homoscedasticity is violated. In the Q-Q residuals plot, the last part of the quantile deviates a lot, indicating the residuals are not normally distributed, violating the assumption of normality. For the Residuals vs. Leverage plot, since there isn't any point with a cook distance greater than 0.5, there is no noticeable point with leverage.

Therefore, we can see that almost every assumption is violated even though the explanatory variables are normal distributed.
...
For the selected model $\sqrt{charges}=42.8299+8.1623bmi+1.5053age-1.8190sex$:
...
Though not perfectly scattered, the residual vs. fitted plots imply the expected value of residuals is roughly 0. Since we did a square root transformation on the response variable, here, we use the standard residuals vs. fitted plot to check the homoscedasticity. Here, since the spread of the points in the plot remains relatively constant across the range of fitted values, then constant variance assumption is satisfied. The QQ plot seems to satisfy the normality assumption since the majority of the data points seem to lie reasonably close to the line, suggesting that the standardized residuals have a distribution that is close to normal. For the Residuals vs. Leverage plot, since there isn't any point with a cook distance greater than 0.5 (we might think point 154 is a high leverage point, but its cook distance is still smaller than 0.5, meaning that it is not a serious point), there is no noticeable point with high cook’s distance and leverage.
...
problem for this model. According to the vif that we have calculated for the model, we can see that all of the vif are smaller than 5, meaning that there isn't serious multicollinearity for our parameters. 
```{r fig.height=2.9, fig.width=10}
par(mfrow=c(1, 4), oma = c(3, 0, 0, 0), mar = c(0, 4, 3, 0.5), cex = 1)
...
mtext("Figure 5", outer = TRUE, side = 1, line = 2, cex = 1, font = 3)
...
kable(table_data, 
      caption = "Variance Inflation Factor for Multicollinearity Analysis")
```
```{r, include = FALSE}
...
The following graph shows the extraction of the data point and the outliers:
...
caption <- textGrob(label = "Figure 8", x = 0.5, y = 0.99, just = "center", gp = gpar(fontsize = 10, fontface = 'italic'))
...
summary(new_model)
```
We then check the assumptions by fitting the model `log_charges ~ age + sex + bmi + children + region` again. As we see from the table, all the factors except BMI are significant. 
...
mtext("Figure 9", outer = TRUE, side = 1, line = 2, cex = 1, font = 3)

# Reset to default parameters
par(mfrow = c(1, 1), oma = c(0, 0, 0, 0))
```
From the Residuals vs. Fitted Values plot, the points are approximately centered around the line y=0, so mean zero assumption is possible to appear to hold. The variability of the points on below the line y=0 is larger than that of the points above the line y=0 on both tails, which indicates that the variance is not constant, so the constant variance assumption does not appear to hold. From the Normal QQ plot, the residuals are slightly deviate from the straight line in the tails, so the normality is possible to be hold. The "Scale-Location" plot shows the spread of residuals, meaning the errors have non-constant variance. Lastly, the "Cook's distance" plot identifies observations 609, 285, and 713 as potential influential points due to their relatively high Cook's distance values, which could disproportionately affect the model's coefficients.
...
\end{align*}
For each bootstrap sample, say $(X_1^{*(\ell)}, Y_1^{*(\ell)}), \ldots, (X_n^{*(\ell)}, Y_n^{*(\ell)})$, we fit the linear regression, leading to a bootstrap estimate of the fitted coefficients $\hat{\beta}_0^{*(\ell)}, \hat{\beta}_1^{*(\ell)}$. Thus, the $B$ bootstrap samples leads to $B$ sets of fitted coefficients. We then estimate the variance by
\begin{align*}
\widehat{\text{Var}}(\hat{\beta}_0) &= \frac{1}{B} \sum_{\ell=1}^{B} (\hat{\beta}_0^{*(\ell)} - \bar{\hat{\beta}}_0)^2, & \bar{\hat{\beta}}_0 &= \frac{1}{B} \sum_{\ell=1}^{B} \hat{\beta}_0^{*(\ell)}, \\
\widehat{\text{Var}}(\hat{\beta}_1) &= \frac{1}{B} \sum_{\ell=1}^{B} (\hat{\beta}_1^{*(\ell)} - \bar{\hat{\beta}}_1)^2, & \bar{\hat{\beta}}_1 &= \frac{1}{B} \sum_{\ell=1}^{B} \hat{\beta}_1^{*(\ell)}, \\
\end{align*}
Thus, we would construct our confidence interval by:
$$
\text{C.I.}(\hat{\beta}_0) = \hat{\beta}_0 \pm z_{1-\alpha/2} \cdot \sqrt{\widehat{\text{Var}}(\hat{\beta}_0)},
$$
$$
\text{C.I.}(\hat{\beta}_1) = \hat{\beta}_1 \pm z_{1-\alpha/2} \cdot \sqrt{\widehat{\text{Var}}(\hat{\beta}_1)}.
$$
...
From the Residuals vs. Fitted Values plot below, the points are centered around the line y=0, so mean zero assumption is possible to appear to hold. the variability of residuals is consistent across all fitted values, which indicates that the variance is constant, so the constant variance assumption appears to hold. From the Normal QQ plot, the residuals are slightly deviate from the straight line in the tails, so the normality is possible to be hold. The "Scale-Location" plot shows the constant spread of residuals, meaning the errors have constant variance. Lastly, the "Cook's distance" plot identifies observations 342, 417, and 554 as potential influential points due to their relatively high Cook's distance values, which could disproportionately affect the model's coefficients.
...
mtext("Figure 10", outer = TRUE, side = 1, line = 2, cex = 1, font = 3)
...
p <- p + labs(caption = "Figure 11: Confidence Intervals for Model Coefficients") +   theme(plot.caption = element_text(face = "italic"))
```
...
data_for_plot <- data.frame(
  ...
  LR_Predicted = predict_charges(data_test)
) %>% 
  pivot_longer(-Actual, names_to = "Model", values_to = "Predicted")
...
plot <- plot + labs(caption = "Figure 12: Comparison of model predictions with actual values") +
  theme(plot.caption = element_text(face = "italic"))
...
From the above analysis, for the smokers, the result showed that bmi and age are strong predictor of smokers' insurance charges. The prediction model shows a linear relationship between the actual and predicted charges and it performs well for the testing daatset. For nonsmokers, our improved method, which included removing outliers and residual scaling, found that age, children, and region are statistically significant predictors, while bmi is not statistically significant predictors. Using the 85% quantile of the empirical residuals in the model, it appears to predict the charges well. These results show that health insurance costs are complicated and that it's important for prediction models to take into account personal traits.
...
Based on what we learned from this research, we should consider more potentional effective factors on health insurance charges, use more advanced predictive modelling methods, and keep improving the models to better reflect how healthcare use and prices change over time. The clear effect that smoking has on insurance rates shows how important it is to have custom models that take living factors into account. As health care needs and insurance policies change, it will be important to use machine learning algorithms and keep models up to date with new data to make predictions more accurate and make sure they stay relevant to current trends.
